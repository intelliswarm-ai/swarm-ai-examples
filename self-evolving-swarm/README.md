...
Demonstrates runtime architecture optimization: the swarm is configured as SEQUENTIAL, but `Swarm.kickoff()` can apply architecture changes recorded from prior runs (switching to PARALLEL when observations support it). No code changes between runs.
...
```
Workflow → ImprovementCollector (observation types)
...
|---------|-------|--------|
| **INTERNAL** | PROCESS_SUITABILITY, EXPENSIVE_TASK, CONVERGENCE_PATTERN, TOOL_SELECTION | EvolutionEngine applies at runtime |
| **EXTERNAL** | FAILURE, ANTI_PATTERN, DECISION_QUALITY, COORDINATION_QUALITY | Recorded locally; submission is opt-in |

Internal observations drive runtime adjustments — the swarm restructures itself using existing capabilities. External observations record structural gaps for later review.
