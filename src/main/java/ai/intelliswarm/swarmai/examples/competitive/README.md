...
```
                     ...
                     COMPETITOR: lines, fans out
                                   |
              +--------------------+--------------------+
              |                    |                    |
              v                    v                    v
   +------------------+ +------------------+ +------------------+
   | AGENT CLONE #1   | | AGENT CLONE #2   | | AGENT CLONE #3   |  ...up to 5
   | Company A        | | Company B        | | Company C        |
   |                  | |                  | |                  |
   | - http_request   | | - web_scrape     | | - calculator     |
   | - calculator     | | - http_request   | | - http_request   |
   | - file_write     | | - file_write     | | - file_write     |
   |                  | |                  | |                  |
   | Generates CODE   | | Reuses skills    | | Reuses skills    |
   | skills (e.g.,    | | from shared      | | from shared      |
   | revenue parser)  | | SkillRegistry    | | SkillRegistry    |
   +--------+---------+ +--------+---------+ +--------+---------+
            |                    |                    |
            +--------------------+--------------------+
                                 |
              ...
              |  Skills generated by any agent are      |
   ...
   +---------------------+            +-------------------------+
   | REVIEW              |            | SYNTHESIS               |
   | [Research Quality   |  drives    | [Chief Strategy Officer]|
   |  Director]          | ---------> | Writes competitive      |
   | Checks completeness |  deeper    | landscape report with   |
   | Issues NEXT_COMMANDS|  analysis  | comparison tables       |
   +---------------------+            +-------------------------+
                                                |
                                                v
